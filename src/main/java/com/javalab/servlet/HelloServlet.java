...
public class HelloServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * - 웹어플리케이션 하나당 하나의 서블릿 컨텍스트 객체가 생성됨.
		...
		// webapp 아래에 있는 /user 폴더의 절대경로를 얻어옴.
		String realPath = sc.getRealPath("/user");
		
		// web.xml에 (설정)한 값으로 서블릿 컨텍스트에 저장되어 있음.
		response.setCharacterEncoding(encoding);
		
		// 응답 인코딩(웹브라우저에 쓸때 한글이 안깨지도록 인코딩)
		// 어떤 형태로 만들어서 사용자에게 전달할건지 컨텍트 타입
		...
		out.println("<h3> 2. web.xml에서 설정 + 서블릿 컨텍스트에 저장된 contentType : " + contentType + "</3>");
